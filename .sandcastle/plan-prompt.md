...
Here are the open issues in the repo. (Use the `dnissimi` gh account in all `gh` commands — **not** `doron-hilma`.)
...
The list above has already been filtered to AFK-ready issues. Issues labelled `ready-for-human`, `needs-info`, or `needs-triage` are excluded by this filter and must never be planned.
